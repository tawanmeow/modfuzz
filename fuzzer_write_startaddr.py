...
def main():
    ...
    if s_block_start("header"):
        # 2-byte tx id
        s_bytes(value=bytes([0x00, 0x01]), size=2, max_len=2, name="tx_id", fuzzable=False)
    ...
    if s_block_start("data"):
        ...
        s_bytes(value=bytes([0x00, 0x00]), size=2, max_len=2, name="ref_num", fuzzable=True)
        # 2-byte number of value
        s_bytes(value=bytes([0x00, 0x00]), size=2, max_len=2, name="value", fuzzable=False)
    s_block_end()
